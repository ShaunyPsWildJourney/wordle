...
export default function Keypad(props) {
  ...
  const { getLetter, handleDelete, handleSubmit , lettersPicked , solution, latestWordle} = props;
  ...
  if (lettersPicked) {
    ...
    let helper = lettersPicked.split('').filter((item) => {
      return solution.solutions[0].includes(item);
    })
    // SET ORANGE LETTERS 
  ...
  let greenery = latestWordle.map((item,index) => {
    return (item === solution.solutions[0][index]) ? item : null;   
  })
  setGreen(prev => prev += greenery) 
  }
}, [lettersPicked, latestWordle, solution.solutions])
